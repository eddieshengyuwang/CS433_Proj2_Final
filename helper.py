...
def gen_test_output(sess, logits, keep_prob, image_pl, data_folder, image_shape):
    ...
    for image_file in glob(os.path.join(data_folder, '*.png')):
        ...
        segmentation = (im_softmax > 0.5).reshape(image_shape[0], image_shape[1]).astype(np.int32)

        street_im = scipy.misc.toimage(segmentation)
...
def get_rotated_images(images, angles):
    ...
    for angle in angles:
        for image in images:
            rotated_images.append(ndimage.rotate(image, angle, mode='reflect', order=0, reshape=False))
