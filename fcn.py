...
from model_utils import greyscale_to_rgb
...
tf.flags.DEFINE_string("graph_dir", os.path.join(PROJECT_DIR, 'graphs', 'kitti'), "Path to graphs")
...
class FCN:
    ...
    def optimize(self, batch_generator, learning_rate=1e-5, keep_prob=1, num_epochs=1, train=False):
    ...
    def _build_placeholders(shape):
        with tf.name_scope('data'):
            ...
            Y = tf.placeholder(tf.int32, [None, shape[0], shape[1], 2], name='Y_placeholder')
